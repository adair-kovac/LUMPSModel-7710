...
class TestSolarRadiation(unittest.TestCase):
    ...
    def test_azimuth_angle(self):
        ...
        self.assertAlmostEqual(math.degrees(azimuth), 207, places=1)
    ...
    def test_flux_with_clouds_no_slope(self):
        date_time = make_date_time(6, 22, 12, vancouver.timezone)
        clouds = calc.Clouds(0, 0, .3)
        incoming_radiation = calc.calc_radiation_flux(date_time, vancouver, clouds=clouds, albedo=.2)
        self.assertAlmostEqual(incoming_radiation, 675.35, places=1)
